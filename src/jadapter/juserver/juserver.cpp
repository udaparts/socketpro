...
std::unordered_map<std::string, HINSTANCE> g_mapPlugin;

template <typename TCHAR>
std::basic_string<TCHAR> ToString(JNIEnv* env, jstring js) {
	std::basic_string<TCHAR> s;
	if (!js)
		return s;
	const jchar* des = env->GetStringChars(js, nullptr);
	s.assign(des, des + env->GetStringLength(js));
	env->ReleaseStringChars(js, des);
	return std::move(s);
}
...
HINSTANCE FindLib(const std::string& pName) {
	HINSTANCE hLib = nullptr;
	SPA::CAutoLock al(g_co);
	auto it = g_mapPlugin.find(pName);
	if (it == g_mapPlugin.end()) {
#ifdef WIN32_64
		hLib = ::LoadLibraryA((pName + ".dll").c_str());
#else
		hLib = ::dlopen(("lib" + pName + "so").c_str(), RTLD_LAZY);
#endif
		g_mapPlugin[pName] = hLib;
	}
	else {
		hLib = it->second;
	}
...
JNIEXPORT jint JNICALL Java_SPA_ServerSide_Plugin_DoSPluginAuthentication(JNIEnv* env, jclass, jstring pName, jlong hSocket, jstring uid, jstring pwd, jint svsId, jstring connString) {
	std::string name = ToString<char>(env, pName);
	if (!name.size()) return -1;
	HINSTANCE hLib = FindLib(name);
	if (!hLib) return -1;
	PDoSPluginAuthentication DoSPluginAuthentication = (PDoSPluginAuthentication)::GetProcAddress(hLib, "DoSPluginAuthentication");
	if (!DoSPluginAuthentication) return -1;
	std::wstring wUID = ToString<wchar_t>(env, uid), wPWD = ToString<wchar_t>(env, pwd), wConn = ToString<wchar_t>(env, connString);
	return DoSPluginAuthentication((SPA::UINT64)hSocket, wUID.c_str(), wPWD.c_str(), (unsigned int)svsId, wConn.c_str());
}

JNIEXPORT jboolean JNICALL Java_SPA_ServerSide_Plugin_SetSPluginGlobalOptions(JNIEnv* env, jclass, jstring pName, jstring jsonOptions) {
	std::string name = ToString<char>(env, pName);
	if (!name.size()) return false;
	HINSTANCE hLib = FindLib(name);
	if (!hLib) return false;
	PSetSPluginGlobalOptions SetSPluginGlobalOptions = (PSetSPluginGlobalOptions)::GetProcAddress(hLib, "SetSPluginGlobalOptions");
	if (!SetSPluginGlobalOptions) return false;
	std::string json = ToString<char>(env, jsonOptions);
	return SetSPluginGlobalOptions(json.c_str());
}

JNIEXPORT jstring JNICALL Java_SPA_ServerSide_Plugin_GetSPluginVersion(JNIEnv* env, jclass, jstring pName) {
	std::string name = ToString<char>(env, pName);
	if (!name.size()) return nullptr;
	HINSTANCE hLib = FindLib(name);
	if (!hLib) return nullptr;
	PGetSPluginVersion GetSPluginVersion = (PGetSPluginVersion)::GetProcAddress(hLib, "GetSPluginVersion");
	if (!GetSPluginVersion) return nullptr;
	return env->NewStringUTF(GetSPluginVersion());
}

JNIEXPORT jstring JNICALL Java_SPA_ServerSide_Plugin_GetSPluginGlobalOptions(JNIEnv* env, jclass, jstring pName, jint estimatedSize) {
	std::string name = ToString<char>(env, pName);
	if (!name.size()) return nullptr;
	HINSTANCE hLib = FindLib(name);
	if (!hLib) return nullptr;
	PGetSPluginGlobalOptions GetSPluginGlobalOptions = (PGetSPluginGlobalOptions)::GetProcAddress(hLib, "GetSPluginGlobalOptions");
	if (!GetSPluginGlobalOptions) return nullptr;
	if (estimatedSize <= 1024) estimatedSize = 1025;
	char* str = (char*)::malloc(estimatedSize);
	if (!str) return nullptr;
	GetSPluginGlobalOptions(str, (unsigned int)estimatedSize);
