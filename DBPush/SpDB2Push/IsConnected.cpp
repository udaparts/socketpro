...
extern "C"
#endif 
void SQL_API_FN Disconnect(SQLUDF_INTEGER *pOut)
{
	if(pOut != NULL)
		*pOut = 0;
...
extern "C"
#endif 
void SQL_API_FN IsConnected(SQLUDF_INTEGER *pOut)
{
	SQLUDF_INTEGER res;
	CAutoLock al(&g_csMySQLPush.m_sec);
	if(g_pClientSocket == NULL)
		...
		res = (g_pClientSocket->IsConnected() ? 1 : 0);
	if(pOut != NULL)
		*pOut = res;
}
...
extern "C"
#endif 
void SQL_API_FN Connect(SQLUDF_VARCHAR *strConnectionString, SQLUDF_INTEGER *pOut)
{
	int res;
	int nPos = 0;
	CStringA strPart;
	...
	if(str.GetLength() == 0)
	{
		...
		if(g_pClientSocket == NULL)
			...
			res = g_pClientSocket->IsConnected() ? 0 : 1;
		if(pOut != NULL)
			*pOut = res;
	}
	...
	int nIndex = 0;
	while (!::PostThreadMessage(g_dwThreadId, WM_DB2_PUSH_CONNECT, 0, 0))
    {
        Sleep(1);
        nIndex++;
        if (nIndex > 5)
