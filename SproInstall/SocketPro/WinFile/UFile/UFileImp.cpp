...
{
	if(nRequestID == idCleanTrack)
	{
		m_tempQueue.CleanTrack();
		m_rdQueue.CleanTrack();
	}
...
STDMETHODIMP CUFile::get_FileNameOrDirectory(BSTR *pVal)
{
	...
	if(pVal)
	{
		...
		if(*pVal)
			::SysFreeString(*pVal);
		*pVal = ::SysAllocString(T2OLE(m_WinFindData.cFileName));
	}
	return S_OK;
}
...
void CUFile::HandleRtn(unsigned short usRequestID, unsigned long ulLen, unsigned long ulLenInBuffer)
{
	...
	if(m_hr != uecOK)
	{
		if(m_tempQueue.GetSize())
		{
			...
			m_bstrErrorMsg.m_str = ::SysAllocStringLen(T2OLE((LPTSTR)m_tempQueue.GetBuffer()), m_tempQueue.GetSize()/sizeof(TCHAR));
			m_tempQueue.SetSize(0);
		}
	...
	case idSearchPath:
		{
			...
			if(ulLen)
			{
				...
				m_bstrFileName.m_str = ::SysAllocStringLen(T2OLE((LPTSTR)m_tempQueue.GetBuffer()), ulLen/sizeof(TCHAR));
				m_tempQueue.Pop(ulLen);
			}
			...
			{
				ATLASSERT((ulLen%sizeof(TCHAR))==0);
				m_bstrFilePart.m_str = ::SysAllocStringLen(T2OLE((LPTSTR)m_tempQueue.GetBuffer()), ulLen/sizeof(TCHAR));
				m_tempQueue.Pop(ulLen);
			}
