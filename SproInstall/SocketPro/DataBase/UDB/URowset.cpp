...
STDMETHODIMP CURowset::AttachSocket(IUnknown *pIUnknownToUSocket)
{
	...
	if(m_pIUSocket.p)
	{
		...
		m_hr = DispEventAdvise(m_pIUSocket.p);
		if(!FAILED(m_hr))
		{
...
STDMETHODIMP CURowset::GetData(short sRow, long lCol, VARIANT *pvtData)
{
	...
	{
		CAutoLock AutoLock(&m_cs.m_sec);
		if(usRow >= m_usBatchSize || ulCol==0 || ulCol>(ULONG)m_aColName.GetSize() || !m_bOpen)
		{
			m_hr = uecUnexpected;
			return m_hr;
		}
...
STDMETHODIMP CURowset::IsBLOB(long lCol, VARIANT_BOOL *pbBLOB)
{
	...
	{
		CAutoLock AutoLock(&m_cs.m_sec);
		if(lCol == 0 || (unsigned long)lCol>(ULONG)m_aColName.GetSize() || !m_bOpen)
		{
			m_hr = uecUnexpected;
			return m_hr;
		}
...
STDMETHODIMP CURowset::GetColName(long lCol, BSTR *pbstrColName)
{
	if(pbstrColName)
	{
		ULONG ulCol = (ULONG)lCol;
		CAutoLock AutoLock(&m_cs.m_sec);
		if(ulCol == 0 || ulCol>(ULONG)m_aColName.GetSize() || !m_bOpen)
		{
			m_hr = uecUnexpected;
			return m_hr;
		}
...
STDMETHODIMP CURowset::GetStatus(short sRow, long lCol, long *plStatus)
{
	...
	{
		CAutoLock AutoLock(&m_cs.m_sec);
		if((unsigned short)sRow>=m_usRecordsFetched || lCol==0 || (unsigned long)lCol>(ULONG)m_aColName.GetSize() || !m_bOpen)
		{
			m_hr = uecUnexpected;
			return m_hr;
		}
...
STDMETHODIMP CURowset::FindColOrdinal(BSTR bstrColName, VARIANT_BOOL bCaseSensitive, long *plCol)
{
	if(plCol)
	{
		*plCol = -1;
		ULONG ul;
		CAutoLock AutoLock(&m_cs.m_sec);
		for(ul=0; ul<(ULONG)m_aColName.GetSize(); ul++)
		{
			if(bCaseSensitive)
			{
...
{
	CAutoLock AutoLock(&m_cs.m_sec);
	if(!m_bOpen || (unsigned short)sRow>=m_usBatchSize || lCol==0 || (unsigned long)lCol>(ULONG)m_aColName.GetSize())
	{
		m_hr = uecUnexpected;
		return m_hr;
	}
...
STDMETHODIMP CURowset::IsWritable(long lCol, VARIANT_BOOL *pbWritable)
{
	...
	{
		CAutoLock AutoLock(&m_cs.m_sec);
		if(!m_bOpen || lCol==0 || (unsigned long)lCol>(ULONG)m_aColName.GetSize())
		{
			m_hr = uecUnexpected;
			return m_hr;
		}
...
unsigned long CURowset::StatusBytes()
{
	...
	for(ul=0; ul<ulSize; ul++)
	{
		...
		ColMeta.m_bBitPos = (BYTE)(ulBits%8);
		ulBits++;
	}
	return ((ulBits+7)/8);
}
...
void CURowset::HandleBLOBIn()
{
	...
	unsigned short usRow = (unsigned short)(m_ulBLOBIndex/m_ulBLOBCount);
...
void CURowset::PackOneRow(unsigned short usRow)
{
	...
	for(ul=0; ul<ulCols; ul++)
	{
		...
		if(IsFixed(ul))
		{
			...
			{
				bLen = (BYTE)(m_aColMeta[ul].m_ulBoundMaxLen);
				m_UQueue.Push(&bLen, 1);
				if(m_aColMeta[ul].m_usBoundDBType == sdVT_VARIANT)
				{
		...
		else
		{
			...
			{
				unsigned short usLen = (unsigned short)(m_aField[ulPos]->m_ulLen);
			...
			{
				BYTE bLen = (BYTE)(m_aField[ulPos]->m_ulLen);
				m_UQueue.Push(&bLen, 1);
				if(bLen != 0xFF)
				{
					m_UQueue.Push(m_aField[ulPos]->m_pData, bLen);
		...
		case sdVT_I2:
			{
				...
				unsigned long ul = (unsigned long)::strlen(strData);
				if(ulLen > ul)
				{
		...
		case sdVT_I2:
			{
				...
				unsigned long ul = (unsigned long)::wcslen(strData);
				if(ulLen > ul)
				{
