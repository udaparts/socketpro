...
STDMETHODIMP CUCommand::Open(BSTR bstrData, long lHint)
{
	...
	for(n=0; n<nCount; n++)
	{
		...
		m_UQueue.Push((unsigned char*)&m_aParamInfo[n].m_usDBType, sizeof(unsigned short));
	}
...
{
	CAutoLock AutoLock(&m_cs.m_sec);
	if(nIndex >= m_aParamInfo.GetSize())
	{
		m_hr = uecUnexpected;
		return m_hr;
	}
...
{
	CAutoLock AutoLock(&m_cs.m_sec);
	if((unsigned long)nIndex >= (unsigned short)m_aOutput.GetSize())
	{
		m_hr = uecUnexpected;
		return m_hr;
	}
	...
	for(n=0; n<nCount; n++)
	{
		...
		if(usDBType == sdVT_BYTES)
		{
			...
			else if(m_aParamInfo[nCol].m_ulLen>(0xFF-1))
			{
				...
				if(m_aParamData[n].vt == (VT_ARRAY|VT_UI1))
				{
					...
					m_UQueue.Push((unsigned char*)&usLen, sizeof(unsigned short));
					::SafeArrayAccessData(m_aParamData[n].parray, (void**)&pData);
		...
		else if(usDBType == sdVT_WSTR)
		{
			...
			else if(m_aParamInfo[nCol].m_ulLen>(0xFF-1))
			{
				...
				if(m_aParamData[n].vt == VT_BSTR)
				{
					usLen = (unsigned short)::SysStringLen(m_aParamData[n].bstrVal)*sizeof(WCHAR);
					m_UQueue.Push((unsigned char*)&usLen, sizeof(unsigned short));
			...
			else
			{
				...
				if(m_aParamData[n].vt == VT_BSTR)
				{
					...
					{
						bLen = (BYTE)::SysStringLen(m_aParamData[n].bstrVal)*sizeof(WCHAR);
					}
					m_UQueue.Push(&bLen, 1);
