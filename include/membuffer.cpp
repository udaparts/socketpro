...
HRESULT VariantChangeType(VARIANT *pvargDest, const VARIANT *pvarSrc, unsigned short wFlags, VARTYPE vt) {
	...
	case VT_BSTR:
		...
		else
		{
			...
			switch (pvarSrc->vt) {
			case VT_I1:
				wsprintf(buffer, L"%d", pvarSrc->cVal);
				break;
			case VT_I2:
				wsprintf(buffer, L"%d", pvarSrc->iVal);
				break;
			case VT_INT:
			case VT_I4:
				wsprintf(buffer, L"%d", pvarSrc->intVal);
				break;
			case VT_I8:
				wsprintf(buffer, L"%lld", pvarSrc->llVal);
				break;
			case VT_UI1:
				wsprintf(buffer, L"%u", pvarSrc->bVal);
				break;
			case VT_UI2:
				wsprintf(buffer, L"%u", pvarSrc->uiVal);
				break;
			case VT_UINT:
			case VT_UI4:
				wsprintf(buffer, L"%u", pvarSrc->uintVal);
				break;
			case VT_UI8:
				wsprintf(buffer, L"%llu", pvarSrc->ullVal);
				break;
			case VT_R4:
				wsprintf(buffer, L"%f", pvarSrc->fltVal);
				break;
			case VT_R8:
				wsprintf(buffer, L"%lf", pvarSrc->dblVal);
				break;
			case VT_BOOL:
				wsprintf(buffer, L"%d", pvarSrc->boolVal);
				break;
			default:
				pvargDest->vt = VT_EMPTY;
				return DISP_E_TYPEMISMATCH;
