#ifndef _UDAPARTS_ODBC_BASE_HANDLER_H_
#define _UDAPARTS_ODBC_BASE_HANDLER_H_
...
namespace SPA {
    namespace ClientSide {
        ...
        class CBaseOdbc : public CAsyncDBHandler<SPA::Odbc::sidOdbc> {
            CBaseOdbc(const CBaseOdbc& ao) = delete;
            CBaseOdbc& operator=(const CBaseOdbc& ao) = delete;

        protected:

            CBaseOdbc(unsigned int svsId, CClientSocket *cs) : CAsyncDBHandler(svsId, cs) {
            }

        public:

            CBaseOdbc(CClientSocket *cs) : CAsyncDBHandler(SPA::Odbc::sidOdbc, cs) {
            }
            ...
            virtual void OnResultReturned(unsigned short reqId, CUQueue &mc) {
                switch (reqId) {
                    case SPA::Odbc::idSQLGetInfo:
                    {
                        ...
                        break;
                    default:
                        CAsyncDBHandler::OnResultReturned(reqId, mc);
                        break;
                }
            }
        ...
        private:
            ...
            void ProcessODBC(const DExecuteResult &handler, CAsyncResult & ar, unsigned short reqId, UINT64 index) {
                ...
                CBaseOdbc *ash = (CBaseOdbc*) ar.AsyncServiceHandler;
                ash->m_csDB.lock();
