...
namespace SPA {
	namespace ClientSide {
		...
		public:
			...
			typedef CStreamingFile* PStreamingFile;
...
#ifdef NODE_JS_ADAPTER_PROJECT
			UINT64 Exchange(Isolate* isolate, int args, Local<Value> *argv, bool download, const wchar_t *localFile, const wchar_t *remoteFile, unsigned int flags) {
				...
				if (args > 1) {
					if (argv[1]->IsFunction()) {
						...
						trans = [this, func, download](CStreamingFile *file, UINT64 transferred) {
							FileCb fcb;
							fcb.Download = download;
				...
				if (args > 2) {
					if (argv[2]->IsFunction()) {
						...
						aborted = [this, func, download](CAsyncServiceHandler *file, bool canceled) {
							...
							tagOperationSystem os = cs->GetPeerOs(&endian);
							fcb.Buffer = CScopeUQueue::Lock(os, endian);
							PStreamingFile f = (PStreamingFile)file;
		...
		private:
			...
			static void file_cb(uv_async_t* handle) {
				...
				CStreamingFile* obj = (CStreamingFile*)handle->data; //sender
				assert(obj);
				if (!obj)
					return;
				...
				while (obj->m_deqFileCb.size()) {
					...
					PStreamingFile processor;
					*cb.Buffer >> processor;
					Local<Function> func = Local<Function>::New(isolate, *cb.Func);
					Local<Object> njFile = NJA::NJFile::New(isolate, processor, true);
