...
public:
    ...
    void ConfirmQueue(unsigned int qHandle, const MQ_FILE::QAttr *qa, size_t count, bool successful);
    ...
    bool SetRouting(unsigned int serviceId0, unsigned int serviceId1);
...
private:
    void HandleServerPingInternal(const boost::posix_time::ptime &tNow);
    ...
    void HandleSlowSwitchInternal(const boost::posix_time::ptime &tNow);
...
private:
    ...
    CErrorCode m_ec;
    std::shared_ptr<CAcceptor> m_pAcceptor;
    std::shared_ptr<CSslContext> m_pSslContext;
