...
void CServer::Recycle(CServerSession *pSession) {
    ...
    if (pSession) {
        if (pSession->m_pRoutingServiceContext != NULL) {
...
bool CServer::StartServerInternal(unsigned int port, unsigned int uiMaxBacklog, bool v6) {
    ...
    do {
        if (m_pAcceptor)
            return false;
        if (m_EncryptionMethod == SPA::TLSv1) {
			if (!m_pSslContext->LoadSecurityLibrary()) {
				m_ec.assign(::GetLastError(),  boost::asio::error::get_system_category());
				break;
			}
			SPA::CScopeUQueue suFile;
			SPA::CScopeUQueue suPassword;
			SPA::Utilities::ToWide(m_strCertFile.c_str(), m_strCertFile.size(), *suFile);
			std::string pwd = GetPassword();
			SPA::Utilities::ToWide(pwd.c_str(), pwd.size(), *suPassword);
			if (!m_pSslContext->OpenStore((const wchar_t*)suFile->GetBuffer(), (const wchar_t*)suPassword->GetBuffer())) {
				m_ec.assign(::GetLastError(),  boost::asio::error::get_system_category());
				break;
			}
			if (m_pSslContext->Open(SP_PROT_SSL3TLS1, m_strPrivateKeyFile.c_str()) != SEC_E_OK)
				break;
        }

        m_pAcceptor.reset(new CAcceptor(m_IoService));
...
bool CServer::SetRouting(unsigned int serviceId0, unsigned int serviceId1) {
...
bool CServer::HasUserId(const wchar_t *userId) {
    ...
    for (it = m_aSession.begin(); it != end; ++it) {
        ...
        if (len != length)
            continue;
        if (boost::iequals(myUserId, userId))
            return true;
    }
    return false;
}
        ...
        if (p->GetLatestTime() < tNow) {
            ...
            switch (sid) {
                ...
                case SPA::sidHTTP:
                    ...
                    break;
                default:
                    p->SendReturnData(SPA::idPing, NULL, 0);
                    break;
            }
        }
    }
...
bool CServer::IsSSLEnabled() {
    CAutoLock sl(m_mutex);
    return (m_EncryptionMethod == SPA::TLSv1);
}
...
unsigned int CServer::StartQueue(const char *qFileName, bool dequeueShared, unsigned int ttl) {
    ...
    if (pwd.size()) {
        ...
        suPwd->SetNull();
        q = std::shared_ptr<MQ_FILE::CMqFile > (new MQ_FILE::CMqFileEx(fn.c_str(), m_nPort, ttl, true,
                (const wchar_t*)suId->GetBuffer(),
                (const wchar_t*)suPwd->GetBuffer(), CServerSession::m_pQLastIndex.get(), false, dequeueShared));

    } else {
        q = std::shared_ptr<MQ_FILE::CMqFile > (new MQ_FILE::CMqFile(fn.c_str(), m_nPort, ttl, true, false, false, dequeueShared));
    }
...
SPA::UINT64 CServer::Enqueue(unsigned int qHandle, unsigned short reqId, const unsigned char *buffer, unsigned int size) {
    ...
    {
        CRAutoLock rsl(m_mQ);
        qIndex = queue->Enqueue(ReqInfo, *sb);
    ...
    SPA::UINT64 msgCount = queue->GetMessageCount();
...
SPA::UINT64 CServer::Dequeue(unsigned int qHandle, USocket_Server_Handle h, unsigned int messageCount, bool beNotifiedWhenAvailable, unsigned int waitTime) {
    ...
    std::vector<unsigned int> vSize;
    std::pair<SPA::UINT64, SPA::UINT64> p;
    unsigned int pos = 0;
    buffer.SetSize(0);
    {
        CRAutoLock rsl(m_mQ);
        vSize = queue->DoBatchDequeue(messageCount, *qAttr, *qRequests, waitTime);
    }
    ...
    for (auto it = vSize.begin(), end = vSize.end(); it != end; ++it) {
        ...
        p.first = qattr[count].MessagePos;
        p.second = qattr[count].MessageIndex;
...
SPA::UINT64 CServer::Dequeue2(unsigned int qHandle, USocket_Server_Handle h, unsigned int maxBytes, bool beNotifiedWhenAvailable, unsigned int waitTime) {
    ...
    {
        CRAutoLock rsl(m_mQ);
        vSize = queue->DoBatchDequeue(*qAttr, *qRequests, maxBytes, waitTime);
    }
    MQ_FILE::QAttr *qattr = (MQ_FILE::QAttr *)qAttr->GetBuffer();

    if (pSession->GetConnIndex() != index) {
        for (auto it = vSize.begin(), end = vSize.end(); it != end; ++it) {
            queue->ConfirmDequeue(qattr[count].MessagePos, qattr[count].MessageIndex, true);
            ++count;
        }
        return 0;
    }
...
void CServer::ConfirmQueue(unsigned int qHandle, const MQ_FILE::QAttr *qa, size_t count, bool successful) {
    CAutoLock sl(m_mQ);
    CMapQueue::iterator it = m_mapQueue.find(qHandle);
    std::shared_ptr<MQ_FILE::CMqFile> &queue = it->second;
    ...
    {
        CRAutoLock rsl(m_mQ);
        queue->ConfirmDequeue(qa, count, !successful);
    }
}
...
bool CServer::EndJob(unsigned int qHandle) {
    CAutoLock sl(m_mQ);
    CMapQueue::iterator it = m_mapQueue.find(qHandle);
    return (it != m_mapQueue.end() && it->second && it->second->EndJob() != INVALID_NUMBER);
}
