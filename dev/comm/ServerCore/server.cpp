...
bool CServer::StartServerInternal(unsigned int port, unsigned int uiMaxBacklog, bool v6) {
    ...
    do {
        ...
        if (IsSsl(method)) {
            ...
            long options = (CSslContext::default_workarounds | CSslContext::no_sslv2 | CSslContext::no_sslv3);
            if (m_strDhFile.size())
...
bool CServer::verify_certificate_cb(bool preverified, boost::asio::ssl::verify_context& ctx) {
    ...
    if (g_pServer->m_cvc) {
        ...
        const char *errMsg = X509_verify_cert_error_string(cts->error);
        boost::scoped_ptr<CUCertImpl> pCert(new CUCertImpl(cert));
        return g_pServer->m_cvc(preverified, depth, cts->error, errMsg, pCert.get());
    }
    return true;
}
        ...
        if ((p->GetLatestTime() + m_ulPingInterval - m_ulTimerElapse - 1000) < tNow) {
            ...
            switch (sid) {
                ...
                case SPA::sidHTTP:
                    ...
                    break;
                default:
                    p->SendReturnData(SPA::idPing, nullptr, 0);
                    break;
            }
        }
    }
...
bool CServer::IsSsl(boost::asio::ssl::context_base::method &method) {
    switch (m_EncryptionMethod) {
        case SPA::TLSv1:
            method = CSslContext::tlsv11_server;
            return true;
            break;
        default:
            break;
