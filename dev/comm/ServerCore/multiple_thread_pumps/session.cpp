...
unsigned int CServerSession::Write(const SPA::CStreamHeader &sh, const unsigned char *s, unsigned int nSize) {
    ...
    if (m_bWBLocked) {
		if (m_pSslSocket) {
			...
			su << sh;
			su->Push(s, nSize);
			CMyOpenSSL::Encrypt(GetSSL(), su->GetBuffer(), su->GetSize(), m_qWrite);
		}
		else {
			m_qWrite << sh;
    ...
    m_bWBLocked = ulLen;
    if (m_pSslSocket) {
        GetSocket().async_write_some(boost::asio::buffer(m_WriteBuffer, ulLen), boost::bind(&CServerSession::OnWriteCompleted, this, nsPlaceHolders::error, nsPlaceHolders::bytes_transferred));
    } else if (m_pSocket) {
...
unsigned int CServerSession::Write(const unsigned char *s, unsigned int nSize) {
    ...
    if (m_bWBLocked) {
		if (m_pSslSocket)
			CMyOpenSSL::Encrypt(GetSSL(), s, nSize, m_qWrite);
		else
			m_qWrite.Push(s, nSize);
        return nSize;
    }
...
void CServerSession::Read() {
    ...
    m_bRBLocked = true;
    if (m_pSslSocket) {
        GetSocket().async_read_some(boost::asio::buffer(m_ReadBuffer, IO_BUFFER_SIZE),
...
bool CServerSession::Route() {
    ...
    if (m_ReqInfo.RequestId == SPA::idRoutingData) {
        ...
        bool valid = m_pServiceContext->IsRoutee(routee);
        if (!valid) {
            m_qRead.Pop(m_ReqInfo.Size);
            m_ReqInfo.RequestId = 0;
            m_ReqInfo.Size = 0;
...
void CServerSession::OnReadCompleted(const CErrorCode& Error, size_t nLen) {
    if (!Error) {
        ...
        if (len) {
			...
			if (m_pSslSocket) {
				m_ccb.m_ulRead += CMyOpenSSL::Decrypt(GetSSL(), m_ReadBuffer, len, m_qRead);
			}
			else {
...
void CServerSession::OnWriteCompleted(const CErrorCode& Error, size_t bytes_transferred) {
    m_ccb.SendTime = (boost::posix_time::microsec_clock::local_time() - g_pServer->m_tStart).total_milliseconds();
    if (Error) {
        CAutoLock sl(m_mutex);
        m_ec = Error;
        CloseInternal();
