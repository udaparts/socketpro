...
bool CServer::StartServerInternal(unsigned int port, unsigned int uiMaxBacklog, bool v6) {
    ...
    bool bSuc = false;
    do {
        if (m_pAcceptor || m_pStartup)
            return false;
        if (IsSsl()) {
			int file_type;
            m_pStartup = new CStartup;
			m_pStartup->ERR_clear_error();
			m_pStartup->SSL_CTX_set_default_passwd_cb(m_pStartup->m_pCtx, CServer::pem_password_cb);
            ...
            if (false/*m_strDhFile.size()*/) {
				/*
                m_pSslContext->use_tmp_dh_file(m_strDhFile, m_ec);
                if (m_ec)
                    break;
				*/
				if (m_pStartup->SSL_CTX_use_certificate_chain_file(m_pStartup->m_pCtx, m_strCertFile.c_str()) != 1) {
					m_ec = CErrorCode(m_pStartup->ERR_get_error(), get_ssl_category());
				}
            } else {
#ifdef WIN32_64
                ...
                if (m_pStartup->SSL_CTX_use_certificate_file(m_pStartup->m_pCtx, m_strCertFile.c_str(), file_type) != 1) {
					m_ec = CErrorCode(m_pStartup->ERR_get_error(), get_ssl_category());
				}
            }
            if (m_ec)
			...
			if (m_strPrivateKeyFile.rfind(".pem") == m_strPrivateKeyFile.size() - 4)
				...
				file_type = SSL_FILETYPE_ASN1;
			if (m_pStartup->SSL_CTX_use_PrivateKey_file(m_pStartup->m_pCtx, m_strPrivateKeyFile.c_str(), file_type) != 1) {
				m_ec = CErrorCode(m_pStartup->ERR_get_error(), get_ssl_category());
			}
            if (m_ec)
                break;
...
int CServer::pem_password_cb(char *buf, int size, int rwflag, void *userdata) {
	if (!buf || size < 2)
		return 0;
	size -= 1;
	SPA::UINT64 src = (SPA::UINT64)g_pServer;
	std::string pwd = MQ_FILE::CMyContainer::Container.Get(src);
	if (pwd.size() < (size_t)size)
		size = (int)pwd.size();
	memcpy(buf, pwd.c_str(), size);
	buf[size] = 0;
	return size;
}
...
unsigned int CServer::StartQueue(const char *qFileName, bool dequeueShared, unsigned int ttl) {
    ...
    if (pwd.size()) {
        ...
        q = std::shared_ptr<MQ_FILE::CMqFile > (new MQ_FILE::CMqFileEx(fn.c_str(), m_nPort, ttl, SPA::oSystemMemoryCached,
                (const wchar_t*)suId->GetBuffer(),
                (const wchar_t*)suPwd->GetBuffer(), CServerSession::m_pQLastIndex.get(), false, dequeueShared));

    } else {
        q = std::shared_ptr<MQ_FILE::CMqFile > (new MQ_FILE::CMqFile(fn.c_str(), m_nPort, ttl, SPA::oSystemMemoryCached, false, false, dequeueShared));
    }
