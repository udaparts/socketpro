...
void CServerSession::ConfirmFailed() {
	...
	for (it = m_mapDequeue.begin(); it != end; ++it) {
		std::vector<MB::U_UINT64> &v = it->second;
		std::vector<MB::U_UINT64>::iterator vi, ve = v.end();
		for (vi = v.begin(); vi != ve; ++vi) {
			g_pServer->ConfirmQueue(it->first, *vi, false);
		}
	}
	m_mapDequeue.clear();
}
...
void CServerSession::OnBaseRequestArrive() {
	{
		...
		switch (m_ReqInfo.RequestId) {
			case MB::idDequeueConfirmed:
			{
				...
				{
					CAutoLock sl(m_mutex);
					assert(m_ReqInfo.Size == (sizeof (qHandle) + sizeof (mqPos)));
					m_qRead >> qHandle >> mqPos;
					m_ReqInfo.Size = 0;
					CQueueMap::iterator it = m_mapDequeue.find(qHandle);
					if (it != m_mapDequeue.end()) {
						std::vector<MB::U_UINT64> &v = it->second;
						std::vector<MB::U_UINT64>::iterator b, end = v.end();
				...
				g_pServer->ConfirmQueue(qHandle, mqPos, true);
			}
				break;
...
void CServerSession::OnRA() {
	...
	switch (m_ReqInfo.RequestId) {
		...
		case MB::idSpeak:
		{
			...
			/*
															{
																			POnChatRequestCame p = m_sc.m_OnChatRequestCame;
																			if(p)
																			{
																							CRAutoLock rl(m_mutex);
																							p(MakeHandler(), (MB::tagChatRequestID)m_ReqInfo.RequestId);
																			}
															}*/
			break;
		default:
...
bool CServerSession::Process(bool bSlowProcessed) {
	...
	while (m_ReqInfo.RequestId == 0 && m_qRead.GetSize() >= sizeof (m_ReqInfo)) {
