...
std::mutex CServerSession::m_mutexBuffer;
std::mutex CServerSession::m_qMutex;
...
bool CServerRegistration::ShouldShutdown() {
    ...
    for (auto it = g_pServer->m_aSession.begin(), end = g_pServer->m_aSession.end(); it != end; ++it) {
        ...
        if (session->m_ClientInfo.SwitchTime > (SPA::UINT64)endTime && session->m_pServiceContext && !session->m_pServiceContext->m_bRegisterred)
            ++badcount;
    }
        ...
        switch (os) {
            case SPA::tagOperationSystem::osWin:
                if (!m_bWin)
                    ++m_nCheapCall;
                break;
            case SPA::tagOperationSystem::osApple:
                if (!m_bApple)
                    ++m_nCheapCall;
                break;
            case SPA::tagOperationSystem::osUnix:
                if (!m_bUnix)
                    ++m_nCheapCall;
                break;
            case SPA::tagOperationSystem::osWinCE:
                if (!m_bWinCe)
                    ++m_nCheapCall;
                break;
            case SPA::tagOperationSystem::osAndroid:
                if (!m_bAndroid)
                    ++m_nCheapCall;
                break;
            default:
...
m_zl(SPA::tagZipLevel::zlDefault),
m_pQBatch(nullptr),
m_bCanceled(false),
...
void CServerSession::EnableClientDequeue(bool enable) {
    SPA::CStreamHeader sh((unsigned short) SPA::tagBaseRequestID::idEnableClientDequeue, sizeof (bool));
    CAutoLock sl(m_mutex);
...
unsigned int CServerSession::SendExceptionResultInternal(const char* errMessage, const char* errWhere, unsigned short requestId, unsigned int errCode) {
    SPA::CScopeUQueue sb;
    if (errMessage)
        SPA::Utilities::ToWide(errMessage, ::strlen(errMessage), *sb);
    return SendExceptionResultInternal((const wchar_t*) sb->GetBuffer(), errWhere, requestId, errCode);
}
...
unsigned int CServerSession::SendExceptionResult(const char* errMessage, const char* errWhere, unsigned short requestId, unsigned int errCode) {
    ...
    if (errMessage)
        SPA::Utilities::ToWide(errMessage, ::strlen(errMessage), *sb);
    return SendExceptionResult((const wchar_t*) sb->GetBuffer(), errWhere, requestId, errCode);
}
...
void CServerSession::Read() {
    if (m_bChatting || m_cs < csSslShaking || g_pServer->m_bStopped)
        return;
...
SPA::CCertificateImpl* CServerSession::GetUCert() {
    return m_pCert.get();
}
...
void CServerSession::OnNonBaseRequestArrive() {
    ...
    //reqId == (unsigned short)SPA::tagBaseRequestID::idInterrupt
    if (m_ReqInfo.Size) {
        m_qRead.Pop(m_ReqInfo.Size);
        m_ReqInfo.Size = 0;
    }
    ...
    switch (m_ReqInfo.RequestId) {
        case (unsigned short) SPA::tagChatRequestID::idSpeak:
        {
            ...
            if (vGroup.size() > 0) {
                ...
                if (crp) {
                    crp(index, SPA::tagChatRequestID::idSpeak);
                }
            }
            sb->SetSize(0);
            sb << vtMsg;
        ...
        case (unsigned short) SPA::tagChatRequestID::idSendUserMessage:
        {
            ...
            USocket_Server_Handle index = MakeHandlerInternal();
            {
                ...
                if (crp) {
                    crp(index, SPA::tagChatRequestID::idSendUserMessage);
                }
            }
            assert(sb->GetSize() == 0);
            sb->SetSize(0);
    ...
    switch (m_ReqInfo.RequestId) {
        case (unsigned short) SPA::tagChatRequestID::idEnter:
        {
            ...
            if (nCount > 0) {
                ...
                if (crp) {
                    crp(index, SPA::tagChatRequestID::idEnter);
                }
            }
            for (n = 0; n < nCount; ++n) {
            ...
            assert(sb->GetSize() >= sizeof (unsigned int));
        {
            ...
            if (vGroup.size() > 0) {
                ...
                if (crp) {
                    crp(index, SPA::tagChatRequestID::idSpeakEx);
                }
            }
            Connection::CConnectionContextBase::ChatGroupsAnd(m_ccb.ChatGroups.data(), (unsigned int) m_ccb.ChatGroups.size(), vGroup.data(), (unsigned int) vGroup.size(), vFinal);
        ...
        case (unsigned short) SPA::tagChatRequestID::idExit:
        {
            ...
            if (nCount > 0) {
                USocket_Server_Handle index = MakeHandlerInternal();
                {
                    ...
                    if (crp) {
                        crp(index, SPA::tagChatRequestID::idExit);
                    }
                }
...
bool CServerSession::Decompress() {
    ...
    if (m_ReqInfo.RequestId == (unsigned short) SPA::tagBaseRequestID::idBatchZipped) {
        if (defaultZipped || fastZip) {
            ...
            assert(ratio > 0);
            res = DecompressRequestTo(ratio, defaultZipped ? SPA::tagZipLevel::zlDefault : SPA::tagZipLevel::zlBestSpeed, m_qRead.GetBuffer(), m_ReqInfo.Size, *sb);
            assert(res > 0);
...
std::mutex CServerSession::m_mutexRouteRequestId;
...
unsigned int CServerSession::GetHTTPCurrentMultiplaxHeaders(SPA::ServerSide::CHttpHeaderValue *HeaderValue, unsigned int count) {
    if (HeaderValue == nullptr || count == 0)
        return 0;
    CAutoLock sl(m_mutex);
    if (!m_pHttpContext || m_pHttpContext->GetCM() == SPA::ServerSide::tagContentMultiplax::cmUnknown)
        return 0;
...
SPA::UINT64 CServerSession::GetHTTPContentLength() {
    ...
    if (!m_pHttpContext)
        return 0;
    if (m_pHttpContext->GetMethod() != SPA::ServerSide::tagHttpMethod::hmPost)
        return 0;
    if (m_pHttpContext->GetTE() != SPA::ServerSide::tagTransferEncoding::teUnknown || m_pHttpContext->GetCM() != SPA::ServerSide::tagContentMultiplax::cmUnknown)
        return 0;
...
bool CServerSession::DownloadFile(const char *filePath) {
    ...
    if (!m_pHttpContext)
        return false;
    if (m_pHttpContext->GetMethod() != SPA::ServerSide::tagHttpMethod::hmGet)
        return false;
