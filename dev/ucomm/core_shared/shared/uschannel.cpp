...
namespace SPA
{
    ...
    CSChannel::CSChannel(bool client, PCredHandle pCredHandle, bool clientCertAuth)
    ...
    void CSChannel::ResetCredHandle(PCredHandle pCredHandle) {
        assert(m_client);
        m_bClientReset = true;
        m_pCredHandle = pCredHandle;
    }

    tagSslHandshakeState CSChannel::GetHandshakeState() const {
        return m_hs;
    }

    bool CSChannel::IsInitialized() const {
    ...
    PCCERT_CONTEXT CSChannel::GetCertContext() const {
        return m_pCertContext;
    }

    PSecHandle CSChannel::GetCtxHandle() const {
        return (PSecHandle)&m_CtxHandle;
    }

    void CSChannel::DeleteCertContext() {
        if (m_pCertContext) {
    ...
    SECURITY_STATUS CSChannel::GetLastStatus() const {
        return m_ss;
    }

    SECURITY_STATUS CSChannel::Encrypt(const unsigned char *buffer, DWORD dwSize, CUQueue & write) {
        SECURITY_STATUS ss;
        assert(m_hs == hsDone);
    ...
    bool CSChannel::Decrypt(const unsigned char *buffer, DWORD dwSize, CUQueue & read) {
        int i;
        bool keep;
        bool backup = false;
        assert(m_hs == hsDone);
    ...
    bool CSChannel::DoHandshake(const unsigned char *buffer, DWORD dwSize, CUQueue & token) {
