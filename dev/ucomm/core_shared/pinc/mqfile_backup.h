...
#pragma pack(push,1)
    ...
    struct QAttr {
        static const SPA::UINT64 QUEUE_RUNNING = 0x8000000000000000;
        ...
        inline bool IsRunning() const {
            return ((MessageIndex & QUEUE_RUNNING) == QUEUE_RUNNING);
        }

        inline void SetRunning(bool running) {
            if (running)
                MessageIndex |= QUEUE_RUNNING;
            else
                MessageIndex &= (~QUEUE_RUNNING);
        }
