...
unsigned char* CServerSession::GetIoBuffer() {
    ...
    if (s == nullptr) {
        s = (unsigned char*) ::malloc(IO_BUFFER_SIZE + IO_ENCRYPTION_PADDING);
    }
    return s;
}
...
void CServerSession::OnBaseRequestArrive() {
    switch (m_ReqInfo.RequestId) {
        ...
        case (unsigned short) SPA::tagBaseRequestID::idEndJob:
            ...
            --m_nJobRequest;
            if (!m_bDequeueTrans) {
                std::cout << "Bad m_bDequeueTrans/(unsigned short)SPA::tagServiceID::idEndJob: " << m_bDequeueTrans << ", " << __FUNCTION__ << " @line: " << __LINE__ << std::endl;
            }
#endif
            m_bDequeueTrans = false;
            break;
        case (unsigned short) SPA::tagBaseRequestID::idStartJob:
            ...
            ++m_nJobRequest;
            if (m_bDequeueTrans) {
                std::cout << "Bad m_bDequeueTrans/(unsigned short)SPA::tagServiceID::idStartJob: " << m_bDequeueTrans << ", " << __FUNCTION__ << " @line: " << __LINE__ << std::endl;
            }
#endif
        ...
        case (unsigned short) SPA::tagBaseRequestID::idCancel:
        {
            ...
            //case (unsigned short)SPA::tagServiceID::idDoEcho:
        default:
        {
            unsigned short id;
            SPA::CScopeUQueue sb;
    ...
    if (!Error) {
        ...
        if (m_pSsl) {
            if (m_pSsl->Done()) {
                ...
                m_bRBLocked = false;
                SPA::CScopeUQueue sb;
                if (m_pSsl->DoHandshake(m_ReadBuffer, len, *sb)) {
                    if (sb->GetSize()) {
                    ...
                    if (m_pSsl->Done()) {
                        CErrorCode ec;
                        OnSslHandShake(ec);
                        if (m_pSsl->Decrypt(nullptr, 0, m_qRead)) {
