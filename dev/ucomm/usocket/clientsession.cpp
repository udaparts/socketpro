...
void CClientSession::TimerHandler() {
    ...
    switch (m_ConnState) {
        case SPA::ClientSide::tagConnectionState::csSslShaking:
...
bool CClientSession::IsSslEnabled() {
    return (m_EncryptionMethod == SPA::tagEncryptionMethod::TLSv1);
}
...
bool CClientSession::SendRequestInternal(CAutoLock &al, unsigned short reqId, const void *pBuffer, unsigned int len) {
    ...
    if (!bQueue && m_qWrite.GetSize() > BLOCK_COUNT * BUFFER_BLOCK_SIZE) {
        if (!IsSameThread()) {
            m_bSendWaiting = true;
...
#ifndef WIN32_64
CClientSession::mutex g_mutexResover;
#endif
...
void CClientSession::OnConnected(const CErrorCode &ec) {
    ...
    if (IsSslEnabled()) {
        ...
        SPA::CScopeUQueue sq;
        bool ok = m_pSsl->DoHandshake(nullptr, 0, *sq);
...
bool CClientSession::GetSockAddr(unsigned int *sockPort, char *strIPAddrBuffer, unsigned short chars) {
    ...
    unsigned short n;
    CAutoLock sl(m_mutex);
    boost::asio::ip::tcp::endpoint ep = m_pSocket->local_endpoint(ec);
    if (ec)
        return false;
    if (sockPort)
        *sockPort = ep.port();
...
bool CClientSession::GetPeerName(unsigned int *sockPort, char *strIPAddrBuffer, unsigned short chars) {
    ...
    unsigned short n;
    CAutoLock sl(m_mutex);
    boost::asio::ip::tcp::endpoint ep = m_pSocket->remote_endpoint(ec);
    if (ec)
        return false;
    if (sockPort)
        *sockPort = ep.port();
...
void CClientSession::SendStopQueueMessage() {
    auto mq = m_qRequest->GetMQInitInfo();
    SPA::CStreamHeader sh((unsigned short) SPA::tagBaseRequestID::idStopQueue, sizeof (mq));
    Write(sh, (const unsigned char*) &mq, sizeof (mq));
}
...
void CClientSession::SendStartQueueMessage() {
    ...
    SPA::CStreamHeader sh((unsigned short) SPA::tagBaseRequestID::idStartQueue, sb->GetSize());
...
void CClientSession::Shutdown(nsIP::tcp::socket::shutdown_type nHow) {
    CErrorCode ec;
    CAutoLock sl(m_mutex);
    if (m_ConnState <= SPA::ClientSide::tagConnectionState::csClosing)
        return;
    m_ConnState = SPA::ClientSide::tagConnectionState::csClosing;
    if (IsContextSet()) {
        /*
        do {
            if (!IsSslEnabled())
...
void CClientSession::OnReadCompleted(const CErrorCode& Error, size_t nLen) {
    ...
    bool b = (m_nRcvBufferSize >= sizeof (SPA::CStreamHeader) || m_ResultInfo.RequestId != 0);
    while (b) {
        m_nRcvBufferSize = m_qRead.GetSize();
            ...
            if (sReqId <= (unsigned short) SPA::tagBaseRequestID::idReservedTwo && !queued && sReqId != (unsigned short) SPA::tagBaseRequestID::idInterrupt) {
                ...
                if (queued && m_pQLastIndex) {
                    ...
                    if ((m_RouterHandle == 0 || m_bRoutingQueueIndexEnabled) && //we don't detect resending if it is for routing
                        ...
                        if (m_ResultInfo.RequestId == (unsigned short) SPA::tagBaseRequestID::idStartJob) {
                            ++m_nJobRequest;
