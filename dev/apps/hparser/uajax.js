SPA.ClientSide = function (host) {
    ...
    me.connect = function (userId, pwd, onOpen, onClose, channel) {
        ...
        function processResponse(done) {
            ...
            var j, p, len, ms = JSON.parse(ws.responseText), rb = [];
            ...
            for (j = 0; j < len; ++j) {
                ...
                if (ms.i) {
                    ...
                    spa.log('Result <<' + p + '>> returned for call index = ' + ms.i);
                }
                else
                    spa.log('Result <<' + p + '>> returned');
            }
        }
        ...
        function cbListen() {
            if (!listening || !listening.xh)
                return;
            switch (listening.xh.readyState) {
                case 2: //HEADERS_RECEIVED
                case 3: //LOADING
                    prevTime = new Date();
                    listening.ok = 1;
                ...
                case 4: //DONE
                    prevTime = new Date();
                    if (listening.xh.status >= 300 || !listening.ok) {
                        onErrorFound(listening.xh.statusText);
                        listening.xh.abort();
                        if (listening.xh.status >= 600 || !listening.ok) {
                            r.close();
                        }
                        return;
                    }
                    ms = listening.xh.responseText;
                    if (!ms)
                        ms = {};
                    else
                        ms = JSON.parse(ms);
        ...
        function subscribe() {
            if (!openAjaxForRequest(listening.xh, cbListen))
                return;
            cIndex = ++callIndex;
            var req = { n: 'ulisten', i: cIndex, v: v, id: id, a: [] };
            listening.xh.send(JSON.stringify(req));
            return cIndex;
        }

        function startListening() {
            if (!listening) {
                listening = { xh: createAjaxObject() };
                subscribe();
            }
        }
