...
namespace MQ_FILE {
    ...
    bool CMqFile::RemoveMsgIndex(SPA::UINT64 pos, SPA::UINT64 nMsgIndex, unsigned int &len) {
        ...
        while ((pos + sizeof (mdh)) <= fileSize) {
            ...
            if (!mdh.IsDequeued()) {
                if (mdh.RequestId == SPA::idStartJob) {
                    ...
                    ++m_msgTransCount;
            } else if (mdh.IsLastDequeued()) {
                balance = 0;
                m_msgCount = 0;
                m_CurrentReadPos = pos;
                m_qaLast.MessagePos = pos;
                m_qaLast.MessageIndex = (mdh.MessageIndex & (~QAttr::QUEUE_DEQUEUED));
                m_qaLast.Len = mdh.Len;
                m_msgTransCount = 0;
            }
            ...
            if (!mdh.IsDequeued()) {
                m_LastTime = mdh.Time;
                if (!m_nMinIndex)
                    m_nMinIndex = (mdh.MessageIndex & (~QAttr::QUEUE_DEQUEUED));
                m_nInternalIndex = (mdh.MessageIndex & (~QAttr::QUEUE_DEQUEUED));
                ++m_msgCount;
            } else
                ++m_nInternalIndex;
        }
    ...
    SPA::UINT64 CMqFile::CancelQueuedRequests(unsigned int startTime, unsigned int endTime) {
        ...
        while ((pos + sizeof (mdh)) <= fileSize) {
            ...
            if (mdh.Time >= endTime)
                break;
            if (!mdh.IsDequeued() && mdh.Time > startTime) {
                RemoveMsgIndex(pos, mdh.MessageIndex, len);
    ...
    bool CMqFile::cmp(const QAttr &a, const QAttr &b) {
        return (a.MessagePos < b.MessagePos);
    }

    void CMqFile::SortDequeued() {
        unsigned int count = m_qDequeued.GetSize() / sizeof (QAttr);
        if (count < 2)
            return;
        QAttr *qa = (QAttr *) m_qDequeued.GetBuffer();
        std::sort(qa, qa + count, cmp);
    }

    bool CMqFile::ShouldCommited() const {
        unsigned int count = m_qDequeued.GetSize() / sizeof (QAttr);
        if (count > 16)
            return true;
        SPA::UINT64 size = 0;
        const QAttr *qa = (const QAttr *) m_qDequeued.GetBuffer();
        for (unsigned int n = 0; n < count; ++n) {
            size += (qa->Len + sizeof (MessageDecriptionHeader));
        }
        return (size >= 8192); //8 * 1024
    }

    unsigned int CMqFile::CommitDequeued() {
        bool ok;
        size_t res;
        SPA::UINT64 index;
#ifndef NDEBUG
        ...
        SortDequeued();
        const QAttr *qa = (const QAttr *) m_qDequeued.GetBuffer();
        const QAttr *qaPrev = qa;
        if (qa->MessagePos == m_qaLast.MessagePos + m_qaLast.Len + sizeof (MessageDecriptionHeader) || !m_qaLast.MessagePos) {
            ++qa;
            ++n;
            while (n < count && qa->MessagePos == qaPrev->MessagePos + qaPrev->Len + sizeof (MessageDecriptionHeader)) {
                qaPrev = qa;
                ++qa;
                ++n;
            }
        }
        qa = (const QAttr *) m_qDequeued.GetBuffer();
        if (n) {
            qa += (n - 1);
			...
			if (mdh.MessageIndex != qa->MessageIndex) {
				std::cout << "---- mdh.MessageIndex = " << mdh.MessageIndex << ", qa->MessageIndex = " << qa->MessageIndex << std::endl;
			}
			ok = SetFilePointer(qa->MessagePos, SEEK_SET);
			assert(ok);
#endif
            index = (qa->MessageIndex | QAttr::QUEUE_DEQUEUED);
    ...
    SPA::UINT64 CMqFile::CancelQueuedRequests(SPA::UINT64 startIndex, SPA::UINT64 endIndex) {
        ...
        while ((pos + sizeof (mdh)) <= fileSize) {
            ...
            assert(res == 1);
            pos += sizeof (mdh);
            if (mdh.GetMessageIndex() > endIndex)
                break;
            if (mdh.GetMessageIndex() >= startIndex) {
                RemoveMsgIndex(pos, mdh.MessageIndex, len);
                ok = SetFilePointer(-16, SEEK_CUR); //-16 == back sizeof (MessageDecriptionHeader)
                mdh.MessageIndex = 0;
                res = fwrite(&mdh, sizeof (mdh), 1, m_hFile);
    ...
    unsigned int CMqFile::CancelQueuedRequests(const unsigned short *ids, unsigned int count) {
        ...
        while (index < count && (pos + sizeof (buffer)) <= fileSize) {
            ...
            MessageDecriptionHeader *mdh = (MessageDecriptionHeader*) buffer;
            SPA::CStreamHeader *sh = (SPA::CStreamHeader*)(buffer + sizeof (MessageDecriptionHeader));
            assert(mdh->MessageIndex < QAttr::QUEUE_DEQUEUED);
            if (mdh->GetMessageIndex()) {
                switch (sh->RequestId) {
                    case SPA::idStartJob:
                        middle = 0;
                        hasStart = true;
                        startPos = pos;
                        startIndex = mdh->GetMessageIndex();
                        break;
                    case SPA::idEndJob:
                    ...
                    default:
                    {
                        ...
                        if (find != vTargetRequest.end()) {
                            ...
                            ++index;
                            RemoveMsgIndex(pos, mdh->GetMessageIndex(), len);
                            mdh->MessageIndex = 0;
                            ...
                            if (sh->RequestId == SPA::idEndJob && hasStart) {
                                hasStart = false;
                                ++index;
                                RemoveMsgIndex(startPos, startIndex, len);
    ...
    void CMqFile::ResetInternal() {
        ...
        if (!m_bCrashSafe && m_hFile)
            fflush(m_hFile);
        bool ok = Truncate(0);
        m_qaLast.MessageIndex = 0;
        m_qaLast.MessagePos = 0;
        m_qaLast.Len = 0;
        m_qDequeued.SetSize(0);
        assert(ok);
    }
    ...
    bool CMqFile::ConfirmDequeue(const QAttr0 *qa, size_t count, bool fail) {
        unsigned int len;
        if (qa == nullptr)
            count = 0;
        bool flush = false;
        ...
        for (size_t n = 0; n < count; ++n) {
            bool found = RemoveMsgIndex(qa[n].MessagePos, qa[n].MessageIndex, len);
            ...
            if (fail) {
                ...
                QAttr qaTemp(qa[n].MessagePos, qa[n].MessageIndex, len);
                m_qDequeued << qaTemp;
                assert(m_msgCount > 0);
                --m_msgCount;
            }
        }
        if ((m_bCrashSafe || m_msgCount == 0 || ShouldCommited()) && CommitDequeued())
            fflush(m_hFile);
        if (m_msgCount == 0 && m_nFileSize > FILE_SIZE_TRUNCATED) {
            assert(m_qOut.GetSize() == 0);
            ResetInternal();
    ...
    bool CMqFile::ConfirmDequeueInternal(SPA::UINT64 pos, SPA::UINT64 index, bool fail) {
        ...
        if (!m_hFile)
            return false;
        bool found = RemoveMsgIndex(pos, index, len);
        if (!found) //it happens possibly if a queue is reset
            return true;
        if (fail) {
    ...
    bool CMqFile::IsNotCommittedYet(SPA::UINT64 nMsgIndex) const {
        if (nMsgIndex <= m_qaLast.MessageIndex)
            return true;
        unsigned int n, Count = m_qDequeued.GetSize() / sizeof (QAttr);
        const QAttr *qa = (const QAttr *) m_qDequeued.GetBuffer();
        for (n = 0; n < Count; ++n) {
            if (qa[n].MessageIndex == nMsgIndex)
                return true;
    ...
    vector<unsigned int> CMqFile::DoBatchDequeue(unsigned int requestCount, SPA::CUQueue &qAttr, SPA::CUQueue &qRequests, unsigned int waitTime) {
        ...
        do {
            ...
            while (m_CurrentReadPos < fileSize) {
                ...
                if (mdh.IsDequeued() || IsInDequeuing(mdh.MessageIndex) || IsNotCommittedYet(mdh.MessageIndex)) {
                    if (mdh.Len) {
                        ok = SetFilePointer(mdh.Len, SEEK_CUR);
                        assert(ok);
                    }
                    if (mdh.IsDequeued()) {
                        QAttr qa(m_CurrentReadPos, mdh.MessageIndex, mdh.Len);
                        m_qDequeued << qa;
                    }
                    ...
                    switch (sh->RequestId) {
                        case SPA::idStartJob:
                        case SPA::idEndJob:
                            assert(attri.MessageIndex < QAttr::QUEUE_DEQUEUED);
                            m_qTransIndex << attri.GetMessageIndex();
                            break;
                        default:
                            break;
                    }
        ...
        do {
            ...
            while (m_CurrentReadPos < fileSize) {
                ...
                if (mdh.IsDequeued() || IsInDequeuing(mdh.MessageIndex) || IsNotCommittedYet(mdh.MessageIndex)) {
                    ...
                    attri.MessagePos = (SPA::UINT64)m_CurrentReadPos;
                    attri.Len = mdh.Len;
                    assert(attri.Len >= sizeof (SPA::CStreamHeader));
                    qAttr << attri;
                    vMdh.push_back(mdh.Len);
                    if (qRequests.GetTailSize() < mdh.Len)
                        qRequests.ReallocBuffer(qRequests.GetSize() + mdh.Len);
...
if (attr.MessageIndex == 1) {
    ...
    QAttr0 CMqFileEx::m_qaTest(0x8765432112345678, 0x8765432112345678);
    ...
    void CQLastIndex::Set(const QueueSha1 &key, const QAttr0 & qa) {
    ...
    QAttr0 CQLastIndex::Seek(const QueueSha1 & key) {
        QAttr0 qa(INVALID_NUMBER, INVALID_NUMBER);
        CAutoLock al(m_cs);
        CMyMap &map = *this;
        CQLastIndex::iterator it = find(key);
        if (it != map.end()) {
