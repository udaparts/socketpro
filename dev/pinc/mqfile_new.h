...
#pragma pack(push, 1)
    ...
    struct QAttr0 {
        ...
        QAttr0(SPA::UINT64 pos, SPA::UINT64 index) : MessagePos(pos), MessageIndex(index) {
        }

        QAttr0() : MessagePos(INVALID_NUMBER), MessageIndex(INVALID_NUMBER) {
        }

        inline bool operator==(const QAttr0 &qa) const {
    ...
    struct QAttr : public QAttr0 {
        static const SPA::UINT64 QUEUE_DEQUEUED = 0x8000000000000000;

        QAttr(SPA::UINT64 pos, SPA::UINT64 index, unsigned int len) : QAttr0(pos, index), Len(len) {
        }
        ...
        inline bool IsDequeued() const {
            return (!MessageIndex || (MessageIndex & QUEUE_DEQUEUED) == QUEUE_DEQUEUED);
        }

        inline bool IsLastDequeued() const {
            return ((MessageIndex & QUEUE_DEQUEUED) == QUEUE_DEQUEUED);
        }

        inline bool IsDequeuedCommittedly() const {
            return (!MessageIndex);
        }

        inline void SetLastDequeued(bool dequeued) {
            if (dequeued)
                MessageIndex |= QUEUE_DEQUEUED;
            else
                MessageIndex &= (~QUEUE_DEQUEUED);
        }

        inline SPA::UINT64 GetMessageIndex() const {
            return (MessageIndex & (~QUEUE_DEQUEUED));
        }
        unsigned int Len;
    };
    ...
    struct CDequeueConfirmInfo {
        ...
        CDequeueConfirmInfo(unsigned int handle, const QAttr0 &qa, bool fail, unsigned short reqId)
        : Handle(handle), QA(qa), Fail(fail), RequestId(reqId) {
        }

        CDequeueConfirmInfo(const QAttr0 &qa, bool fail, unsigned short reqId)
...
#pragma pack(pop)
    ...
    class CMqFile {
#ifndef NDEBUG
        unsigned int m_nJobBalanceDequeue;
        unsigned int m_nJobBalanceEnqueue;
#endif
...
#pragma pack(push,1)
        ...
        struct MessageDecriptionHeader {
            ...
            inline bool IsDequeued() const {
                return (!MessageIndex || (MessageIndex & QAttr::QUEUE_DEQUEUED) == QAttr::QUEUE_DEQUEUED);
            }

            inline bool IsLastDequeued() const {
                return ((MessageIndex & QAttr::QUEUE_DEQUEUED) == QAttr::QUEUE_DEQUEUED);
            }
            ...
            inline void SetLastDequeued(bool dequeued) {
                if (dequeued)
                    MessageIndex |= QAttr::QUEUE_DEQUEUED;
                else
                    MessageIndex &= (~QAttr::QUEUE_DEQUEUED);
            }

            inline SPA::UINT64 GetMessageIndex() const {
                return (MessageIndex & (~QAttr::QUEUE_DEQUEUED));
            }
        };
...
#pragma pack(pop)
    ...
    public:
        CMqFile(const char *fileName, unsigned short port, unsigned int ttl, SPA::tagOptimistic crashSafe, bool secure, bool client = false, bool shared = false);
        //CMqFile(CMqFile && mq);
    ...
    public:
        ...
        bool ConfirmDequeue(const QAttr0 *qa, size_t count, bool fail = false);
    ...
    private:
        ...
        inline bool RemoveMsgIndex(SPA::UINT64 pos, SPA::UINT64 nMsgIndex, unsigned int &len);
        ...
        inline bool IsNotCommittedYet(SPA::UINT64 nMsgIndex) const;
        static bool cmp(const QAttr &a, const QAttr &b);
        void SortDequeued();
        unsigned int CommitDequeued();
        bool ShouldCommited() const;

    protected:
        static unsigned char empty[PAD_EMPTY_SIZE];

    private:
        SPA::CScopeUQueue m_suDequeued;
    ...
    private:
        static QAttr0 m_qaTest;

    public:
        CMqFileEx(const char *fileName, unsigned short port, unsigned int ttl, SPA::tagOptimistic crashSafe, const wchar_t *userId, const wchar_t *password, CQLastIndex *pLastIndex, bool client = false, bool shared = false);
    ...
    typedef boost::unordered_map<QueueSha1, QAttr0, qs_hash, qs_equal> CMyMap;
    ...
    public:
        ...
        bool IsAvailable();
        inline bool IsDirty();
        void Set(const QueueSha1 &key, const QAttr0 &qa);
        void Remove(const QueueSha1 &key);
        QAttr0 Seek(const QueueSha1 &key);
    ...
    extern SPA::CUCriticalSection g_csQFile;
