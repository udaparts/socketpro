...
namespace rapidjson {
    ...
    const unsigned int DEFAULT_CHUNK_SIZE = 4 * 1024;
    ...
    public:
        ...
        static void* Malloc(size_t size) {
            if (size <= DEFAULT_CHUNK_SIZE + sizeof (ChunkHeader)) {
                SPA::CAutoLock al(_cs);
                size_t size = _vBuffer.size();
                if (size) {
                    void *p = _vBuffer[size - 1];
                    _vBuffer.pop_back();
                    return p;
                }
            }
        ...
        static void Free(void *ptr) {
            if (ptr) {
                ...
                if (ch->capacity > DEFAULT_CHUNK_SIZE)
                    ::free(ptr);
                else {
                    SPA::CAutoLock al(_cs);
        ...
        static void Release() {
            ...
            //It may be error in resetting size 
            //while some buffers are still used within rapidjson::GenericDocument<Encoding, Allocator> objects.
            //However, it is not a real problem at all.
            _allocatedSize = 0;
        }

    private:
        static SPA::CUCriticalSection _cs;
        static std::vector<void*> _vBuffer;
        static size_t _allocatedSize;
    };
    ...
    public:
        ...
        chunkHead_(0), chunk_capacity_(chunkSize) {
        ...
        static const unsigned int kDefaultChunkCapacity = DEFAULT_CHUNK_SIZE; //64 * 1024; //!< Default chunk capacity.
    ...
    private:
        ...
        void AddChunk(size_t capacity) {
            ChunkHeader* chunk = (ChunkHeader*) CrtAllocator::Malloc(sizeof (ChunkHeader) + capacity);
