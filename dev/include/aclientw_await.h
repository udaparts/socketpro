...
namespace SPA {
    namespace ClientSide {
        ...
        public:
            ...
            public:
                typedef std::tr1::function<void(CClientSocket*, const CMessageSender&, const SPA::UVariant&) > DOnSendUserMessage;
                DOnSendUserMessage OnSendUserMessage;

                typedef std::tr1::function<void(CClientSocket*, const CMessageSender&, const unsigned char*, unsigned int) > DOnSendUserMessageEx;
                DOnSendUserMessageEx OnSendUserMessageEx;

                typedef std::tr1::function<void(CClientSocket*, const CMessageSender&, const unsigned int*, unsigned int, const SPA::UVariant&) > DOnPublish;
...
#else
                typedef std::tr1::function<void(CClientSocket*, const CMessageSender&, const unsigned int*, unsigned int, const unsigned char*, unsigned int) > DOnPublishEx;
#endif
                DOnPublishEx OnPublishEx;

                typedef std::tr1::function<void(CClientSocket*, const CMessageSender&, const unsigned int*, unsigned int) > DOnSubscribe;
                DOnSubscribe OnSubscribe;

                typedef std::tr1::function<void(CClientSocket*, const CMessageSender&, const unsigned int*, unsigned int) > DOnUnsubscribe;
        ...
        public:
            ...
            typedef std::tr1::function<void(CClientSocket*, int) > DSocketEvent;
            DSocketEvent SocketClosed;
            DSocketEvent HandShakeCompleted;
            DSocketEvent SocketConnected;

            typedef std::tr1::function<void(CClientSocket*, unsigned short) > DRequestEvent;
            DRequestEvent BaseRequestProcessed;
            DRequestEvent AllRequestsProcessed;

            typedef std::tr1::function<void(CClientSocket*, unsigned short, CUQueue &) > DRequestProcessed;
...
#else
            typedef std::tr1::function<void(CClientSocket*, unsigned short, const wchar_t*, const char*, unsigned int) > DExceptionFromServer;
#endif
        ...
        public:
            ...
            typedef std::tr1::function<void(CAsyncServiceHandler*, unsigned short, CUQueue&) > DResultReturned;
            typedef std::tr1::function<void(CAsyncServiceHandler*, unsigned short requestId, const wchar_t *errMessage, const char* errWhere, unsigned int errCode) > DServerException;
		...
		public:
            std::future<void> async(unsigned short reqId, const unsigned char *pBuffer, unsigned int size) {
                ...
                if (!SendRequest(reqId, pBuffer, size, NULL_RH)) {
                    throw CUException(GetAttachedClientSocket()->GetErrorMsg().c_str(), __FILE__, __LINE__, __FUNCTION__, GetAttachedClientSocket()->GetErrorCode());
                } else {
                    prom->set_value();
                }
		...
		private:
			struct IUAsyncCancel
			{
				virtual void set_uexception(std::exception_ptr p) = 0;
			};
			template<typename R>
			class CUPromise : public IUAsyncCancel, public std::promise<R>
			{
			public:
				void set_uexception(std::exception_ptr p) {
					std::promise<R>::set_exception(p);
				}
			};
		...
		public:
			...
			std::future<R> async(unsigned short reqId, const unsigned char *pBuffer, unsigned int size) {
...
#else
				CUPromise<R> *up = new CUPromise<R>;
				{
					m_cs.lock();
					m_deqUAsyncCancel.push_back(up);
					m_cs.unlock();
					if (IsBatching()) {
						m_cs.lock();
						++m_nAsyncBatching;
                ...
                std::shared_ptr<CUPromise<R> > prom(up, [this](CUPromise<R> *p) {
					{
						CAutoLock al(this->m_cs);
						for (auto it = this->m_deqUAsyncCancel.begin(), end = this->m_deqUAsyncCancel.end(); it != end; ++it) {
							if (*it == p) {
								this->m_deqUAsyncCancel.erase(it);
								break;
							}
						}
					}
        ...
        private:
#if defined(_FUTURE_) || defined(_GLIBCXX_FUTURE)
			size_t m_nAsyncBatching;
			std::deque<IUAsyncCancel*> m_deqUAsyncCancel;
#endif
        ...
        public:
            ...
            : m_ash(ash), m_br(nullptr), m_bw(nullptr), m_nDownloadFileSize(~0), m_nPos(0) {
                m_ash.ResultReturned = [this](CAsyncServiceHandler *ash, unsigned short reqId, CUQueue & q) {
                    switch (reqId) {
            ...
            typedef std::tr1::function<void(const CStreamHelper*, UINT64 pos) > DProgress;
            DProgress Progress;
        ...
        public:
            ...
            std::wstring DownloadFile(std::ostream &bw, const std::wstring& RemoteFilePath) {
                ...
                bool ok = (m_ash.SendRequest(CStreamSerializationHelper::idStartDownloading, RemoteFilePath, [this, &res](CAsyncResult & ar) {
                    ...
                    if (this->Progress && this->m_nDownloadFileSize != (~0) && res.size() == 0 && this->m_bw)
        ...
        public:
            ...
            typedef std::tr1::function<void(CSocketPool*, tagSocketPoolEvent, THandler*) > DSocketPoolEvent;
        ...
        public:
            ...
            bool StartSocketPool(const CConnectionContext &cc, unsigned int socketsPerThread, unsigned int threads = 0, bool avg = true, tagThreadApartment ta = taNone) {
                ...
                for (n = 0; n < threads; ++n) {
                    ...
                    for (j = 0; j < socketsPerThread; ++j) {
                        ppCCs[n][j] = cc;
                    }
                }
                ok = PostProcess(ppCCs);
                for (n = 0; n < threads; ++n) {
                    delete[](ppCCs[n]);
                }
                delete[]ppCCs;
        ...
        private:
            ...
            bool PostProcess(CConnectionContext **ppCCs) {
                bool ok;
                unsigned int socketsPerThread;
                unsigned int pos = 0;
                bool first = true;
                CAutoLock al(m_cs);
                unsigned int poolId = m_nPoolId;
                size_t size = m_mapSocketHandler.size();
                socketsPerThread = ClientCoreLoader.GetSocketsPerThread(m_nPoolId);
                for (auto it = m_mapSocketHandler.begin(), end = m_mapSocketHandler.end(); it != end; ++it) {
            ...
            static void CALLBACK SPE(unsigned int poolId, SPA::ClientSide::tagSocketPoolEvent spe, USocket_Client_Handle h) {
                ...
                switch (spe) {
                    case speTimer:
                        /*
                        std::cout << "Timer running for " << poolId << ", thread id = ";
                        #ifdef WIN32_64
