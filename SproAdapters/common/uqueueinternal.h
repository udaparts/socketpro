...
//#pragma warning(disable: 4244) // warning C4244: 'argument' : conversion from 'ULONG' to 'const unsigned short', possible loss of data

class CInternalUQueue
{
public:
	CInternalUQueue(unsigned long ulMaxLen = DEFAULT_INITIAL_QUEUE_SIZE, unsigned long ulBlockSize = DEFAULT_UQUEUE_BLOCK_SIZE)
	{
		m_ulMaxBuffer = ulMaxLen;
		if(ulBlockSize == 0)
		{
			m_ulBlockSize = DEFAULT_UQUEUE_BLOCK_SIZE;
		}
		else
	...
	//no copy constructor
	CInternalUQueue(const CInternalUQueue &UQueue);
	//no assignment operator 
	CInternalUQueue& operator=(const CInternalUQueue &UQueue);

public:
	inline const unsigned char* const GetBuffer(unsigned long ulHead = 0) 
	{
		if(ulHead >= m_ulLen)
			ulHead = m_ulLen;
	...
	inline unsigned long GetMaxSize()
	{
		return m_ulMaxBuffer;
	}

	inline unsigned long GetBlockSize()
	{
	...
	inline unsigned long GetHeadPosition()
	{
		return m_ulHeadPos;
	}
	...
	{
		if(m_pBuffer != NULL && m_ulMaxBuffer > 0)
		{
			if(m_ulHeadPos > 0)
			{
	...
	*/
	virtual void Insert(const char* str, unsigned long ulLen, unsigned long ulPos=0)
	{
		if(str)
		{
			if(!ulLen)
				ulLen = (unsigned long)strlen(str);
	...
	*/
	virtual void Insert(const wchar_t* str, unsigned long ulLen = 0, unsigned long ulPos = 0)
	{
		if(str)
		{
			if(!ulLen)
				ulLen = (unsigned long)::wcslen(str);
	...
	*/
	...
	{
		if(pData == NULL || ulBufferLen == 0 || m_ulLen==0 || m_pBuffer == NULL)
			return 0;
		if(ulPos >= m_ulLen)
			return 0;
		if(ulBufferLen > (m_ulLen - ulPos))
		...
		case VT_BSTR:
			{
				...
				else if(ulLen)
				{
					if(ulLen > GetSize() || (ulLen%sizeof(wchar_t)) > 0)
						throw std::exception("Invalid unicode string found!");
		...
		default:
			if((vtData.vt & VT_ARRAY) == VT_ARRAY)
			{
				...
				case (VT_DECIMAL|VT_ARRAY):
					...
					break;
				default:
					throw std::exception("Unknown data type found!");
					break;
				}
				vtData.parray = psa;
			}
	...
	{
		return Pop((unsigned char*)pData, (unsigned long)sizeof(ctype), ulPos);
	}
	...
	*/
	...
	template <class ctype>
	void PushWithSize(const ctype *pData, unsigned long ulSize)
	{
		ULONG ulPos = GetSize();
	...
	*/
	CInternalUQueue& operator << (const CComBSTR &bstr )
	{
		ULONG ulSize;
		if(bstr.m_str == NULL)
			ulSize = UQUEUE_SIZE_NULL;
		...
		if(ulSize != UQUEUE_SIZE_NULL && ulSize != 0)
		{
			Push((BYTE*)bstr.m_str, ulSize);
		}
		return *this;
	}

	CInternalUQueue& operator >> (CComBSTR &bstr)
	{
		ULONG ulSize;
	...
	CInternalUQueue& operator<<(const VARIANT &vtData)
	{
		PushVT(vtData);
		return *this;
	}

	CInternalUQueue& operator>>(VARIANT &vtData)
	{
		PopVT(vtData);
		return *this;
	}

	CInternalUQueue& operator<<(const CComVariant &vtData)
	{
		PushVT(vtData);
		return *this;
	}

	CInternalUQueue& operator>>(CComVariant &vtData)
	{
		PopVT(vtData);
		return *this;
	}
	...
	CInternalUQueue& operator << (LPCSTR strData)
	{
		ULONG ulSize;
		if(strData == NULL)
		{
		...
		{
			ulSize = (unsigned long)::strlen(strData);
		}
		Push(&ulSize);
		if(ulSize != UQUEUE_SIZE_NULL && ulSize != 0)
		{
			Push((const unsigned char*)strData, ulSize);
		}
		return *this;
	}

	CInternalUQueue& operator << (LPCWSTR strData)
	{
		ULONG ulSize;
		if(strData == NULL)
		{
		...
		{
			ulSize = (unsigned long)::wcslen(strData)*sizeof(WCHAR);
		}
		Push(&ulSize);
		if(ulSize != UQUEUE_SIZE_NULL && ulSize != 0)
		{
	...
	CInternalUQueue& operator << (const unsigned short &data)
	{
...
*/
template<class ctype>
CInternalUQueue& operator << (CInternalUQueue &UQueue, const ctype &data)
{
	UQueue.Push(&data);
	return UQueue;
}
...
template<class ctype>
CInternalUQueue& operator >> (CInternalUQueue &UQueue, ctype &data)
{
