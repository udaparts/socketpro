...
#define VT_TIMESPAN				(0xC00)
#define VT_DATETIMEOFFSET		(0xB00)

//#pragma warning(disable: 4947) // System::Runtime::InteropServices::FILETIME

namespace SocketProAdapter
{
	CUQueue::CUQueue() : m_bScope(false), m_ms(nullptr)
	{
		m_pUQueue = new CInternalUQueue();
	}

	CUQueue::CUQueue(int nBlockSize) : m_bScope(false), m_ms(nullptr)
	{
		if(nBlockSize <= 0)
			nBlockSize = 1024;
	...
	{
		if(nSize < 0)
			throw gcnew Exception(gcnew String("Bad input (nSize < 0)."));
	...
	{
		if(nPos < 0)
			throw gcnew Exception(gcnew String("Bad input (nPos < 0)."));
...
/*
	...
	void CUQueue::Push(array<BYTE> ^aData, int nLen, int nIndex)
	{
		if(aData != nullptr && nLen != 0)
		{
			if(nLen < 0 || nIndex < 0 || nIndex > aData->Length -1)
				throw gcnew Exception(gcnew String("Bad input with wrong nLen or nIndex."));
	...
	void CUQueue::Push(array<BYTE> ^aData, int nLen)
	{
		...
		{
			if(nLen < 0)
				throw gcnew Exception(gcnew String("Bad input (nLen < 0)."));
	...
	void CUQueue::Push(TimeSpan ts)
	{
		__int64 ticks = ts.Ticks;
		Push(ticks);
	}
	
	void CUQueue::Push(DateTimeOffset offset)
	{
		Push(offset.UtcDateTime);
	}
	...
	void CUQueue::Push(Object ^obData, bool bToNative, bool bDecToCY)
	{
		...
		if(t->IsArray)
		{
			...
			else if(t->FullName == "System.TimeSpan[]")
			{
				...
				vt |= VT_TIMESPAN;
				m_pUQueue->Push(&vt);
				array<TimeSpan> ^aNull = (array<TimeSpan> ^)obData;
				int nSize = aNull->Length;
				Push(nSize);
				for(nIndex = 0; nIndex<nSize; nIndex++)
				{
					Push(aNull[nIndex].Ticks);
				}
			}
			else if(t->FullName == "System.DateTimeOffset[]")
			{
				int nIndex;
				vt |= VT_DATETIMEOFFSET;
				m_pUQueue->Push(&vt);
				array<DateTimeOffset> ^aNull = (array<DateTimeOffset> ^)obData;
				int nSize = aNull->Length;
				Push(nSize);
				for(nIndex = 0; nIndex<nSize; nIndex++)
				{
					Push(aNull[nIndex].UtcDateTime);
				}
			}
			else
			{
		...
		else
		{
			if(t->IsPrimitive)
			{
				...
				else
				{
					...
					str += t->FullName;
					str += ") not supported for serialization.";
			...
			else
			{
				...
				else if(t == TimeSpan::typeid)
				{
					vt = VT_TIMESPAN;
					Push(vt);
					Push(((TimeSpan^)obData)->Ticks);
				}
				else if(t == DateTimeOffset::typeid)
				{
					vt = VT_DATETIMEOFFSET;
					Push(vt);
					Push(((DateTimeOffset^)obData)->UtcDateTime);
				}
				else
				{
	...
	int CUQueue::XmlDeserialize(T %obj)
	{
		...
		if(m_pUQueue->GetSize() < sizeof(bool))
			throw gcnew Exception(gcnew String("Invalid XML found during de-serialization.") + MAKE_BALANCE);
	...
	int CUQueue::Pop(CUQueue ^%UQueue)
	{
		...
		if(ulSize != UQUEUE_SIZE_NULL)
		{
			if(ulSize > m_pUQueue->GetSize())
			{
				...
				m_pUQueue->SetSize(0);
				throw gcnew Exception(gcnew String("Invalid CUQueue found during de-serailization.") + MAKE_BALANCE);
			}
			if(UQueue == nullptr)
			{
	...
	int CUQueue::Pop([Out]TimeSpan %ts)
	{
		__int64 ticks;
		int size = Pop(ticks);
		ts = TimeSpan(ticks);
	...
	int CUQueue::Pop([Out]DateTimeOffset %offset)
	{
		DateTime dt;
		int size = Pop(dt);
		offset = DateTimeOffset(dt);
		return size;
	}
	...
	int CUQueue::Pop(String ^%strData, int nBytes)
	{
		if(nBytes < 0 || (nBytes%sizeof(wchar_t)) != 0)
			throw gcnew Exception(gcnew String("Bad input with wrong input nBytes."));
		strData = nullptr;
		ULONG ulLen = (m_pUQueue->GetSize()/2)*2;
	...
	int CUQueue::PopCY(Decimal %cyMoney)
	{
		...
		if(nGet < sizeof(lData))
			throw gcnew Exception(gcnew String("Invalid currency found during de-serailization.") + MAKE_BALANCE);
		cyMoney = lData;
		cyMoney /= 10000;
		return nGet;
	}
	...
	int CUQueue::PopVariantBool(bool %b)
	{
		...
		if(nGet < sizeof(usData))
			throw gcnew Exception(gcnew String("Invalid boolean found during de-serailization.") + MAKE_BALANCE);
	...
	int CUQueue::PopSystemTime(DateTime %dt)
	{
		...
		if(m_pUQueue->Pop(&sysTime) < sizeof(sysTime))
			throw gcnew Exception(gcnew String("Invalid DateTime found during de-serailization.") + MAKE_BALANCE);
...
/*
	DateTime^ CUQueue::PopSystemTime()
	{
		SYSTEMTIME sysTime;
		if(m_pUQueue->Pop(&sysTime) < sizeof(sysTime))
			throw gcnew Exception(gcnew String("Invalid data available."));
	...
	int CUQueue::Pop(Object ^%obData)
	{
		...
		if(lLen < sizeof(vt))
			throw gcnew Exception(gcnew String("Invalid data found during de-serailization.") + MAKE_BALANCE);
		if(vt == VT_EMPTY)
		{
			obData = nullptr;
			return lLen;
		...
		if((vt & VT_ARRAY) == VT_ARRAY)
		{
			...
			else
			{
				...
				case (VT_DOTNET_OBJECT|VT_ARRAY):
					...
					break;
				case (VT_TIMESPAN|VT_ARRAY):
					{
						ULONG ul;
						array<TimeSpan> ^pData = gcnew array<TimeSpan> (ulSize);
						for(ul=0; ul<ulSize; ul++)
						{
							__int64 tick;
							lLen += Pop(tick);
							pData[ul] = TimeSpan(tick);
						}
						obData = pData;
					}
					break;
				case (VT_DATETIMEOFFSET|VT_ARRAY):
					{
						ULONG ul;
						array<DateTimeOffset^> ^pData = gcnew array<DateTimeOffset^> (ulSize);
						for(ul=0; ul<ulSize; ul++)
						{
							DateTime dt;
							lLen += Pop(dt);
							pData[ul] = gcnew DateTimeOffset(dt);
						}
						obData = pData;
					}
					break;
				...
				default:
					...
					String ^str = "Data type (";
					str += vt.ToString();
					str += ") not supported for de-serialization.";
					str += MAKE_BALANCE;
					throw gcnew Exception(str);
					break;
				}
		...
		else
		{
			...
			case VT_DOTNET_OBJECT:
				lLen += Deserialize(obData);
				break;
			case VT_TIMESPAN:
				{
					__int64 tick;
					lLen += Pop(tick);
					obData = gcnew TimeSpan(tick);
				}
				break;
			case VT_DATETIMEOFFSET:
				{
					DateTime dt;
					lLen += Pop(dt);
					obData = gcnew DateTimeOffset(dt);
				}
				break;
	...
	int CUQueue::Deserialize(T %obj, bool bSoapFormat, IFormatter ^formatter)
	{
		...
		{
			Type ^t = T::typeid;
			String ^errMsg = gcnew String("Invalid ") + t->FullName + " found during de-serialization.";
			throw gcnew Exception(errMsg + MAKE_BALANCE);
		}
	...
	int CUQueue::Load([Out]String ^%str)
	{
		str = nullptr;
		unsigned int unLen;
		int nGet = m_pUQueue->Pop(&unLen);
		if(nGet < sizeof(unLen))
			throw gcnew Exception(gcnew String("Invalid string found during de-serialization.") + MAKE_BALANCE);
	...
	{
		m_UQueue->m_bScope = true;
	}
	CScopeUQueue::~CScopeUQueue()
	{
		m_UQueue->m_bScope = false;
		Unlock(m_UQueue);
	}
	...
	void CScopeUQueue::Unlock(CUQueue ^UQueue)
	{
		...
		{
			if(UQueue->m_bScope)
				throw gcnew System::InvalidOperationException("Can't unlock the member UQueue of CScopeUQueue directly");
			UQueue->SetSize(0);
			g_cs.Lock();
