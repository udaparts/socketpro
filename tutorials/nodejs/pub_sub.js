...
const idSleep = idSayHello + 1; const idEcho = idSleep + 1;

var cs = SPA.CS; cs.TLS.CA = 'ca.cert.pem'; //1 'root' on windows for server certificate authentication

//create a socket pool object
var p = cs.newPool(sid); global.p = p;
...
var cc = cs.newCC('localhost', 20901, 'root', 'Smash123', 0); //2 0 = NoEncryption or 1 = TLSv1.x
...
var msg = hw.Socket.Push; //3

//streaming all the following four requests and two messenger message requests
...
ok = hw.SendRequest(idEcho, SPA.newBuffer().Save(q => {
    ...
    console.log({
        nullStr: q.LoadString(), objNull: q.LoadObject(), aDate: q.LoadDate(), aDouble: q.LoadDouble(),
        aBool: q.LoadBool(), unicodeStr: q.LoadString(), asciiStr: q.LoadAString(), objBool: q.LoadObject(),
        objString: q.LoadObject(), objArrString: q.LoadObject(), objArrInt: q.LoadObject()
    });
});

(async (hw, fName, lName) => { //4
    try {
...
ok = msg.SendUserMessage('some_user_id', 'A test message from node.js'); //5

//send a message to three groups of connected clients
ok = msg.Publish('A test publish message from node.js', [1, 3, 7]); //6
