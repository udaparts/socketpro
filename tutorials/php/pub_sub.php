<?php
        ...
        const idReservedTwo = SPA\BaseID::idReservedTwo; //Your request id must larger than 0x2001
//Request ids must be the same ones at server side, which could be implemented with one of C/C++, C#/VB.NET, Java and Python languages
        const idSayHello = idReservedTwo + 1;
        const idSleep = idReservedTwo + 2;
        const idEcho = idReservedTwo + 3;
...
try {
    do {
        ...
        if (!$messenger->SendUserMessage('A test message from PHP', 'some_user_id')) {
            echo 'Session closed<br/>';
            break;
        }
        if (!$messenger->Publish('A test message from PHP', array(1, 3, 7))) {
            echo 'Session closed<br/>';
            break;
        }
