...
(async () => {
    try {
        ...
        var fCt0 = db.execute('CREATE TABLE COMPANY(ID INT8 PRIMARY KEY NOT NULL,name CHAR(64)NOT NULL,ADDRESS varCHAR(256)not null,Income float not null)');
        var fCt1 = db.execute("CREATE TABLE EMPLOYEE(EMPLOYEEID INT8 PRIMARY KEY NOT NULL unique,CompanyId INT8 not null,name NCHAR(64)NOT NULL,JoinDate DATETIME not null default(datetime('now')),IMAGE BLOB,DESCRIPTION NTEXT,Salary real,FOREIGN KEY(CompanyId)REFERENCES COMPANY(id))");
        var fd = db.execute('delete from employee;delete from company');
        console.log(await Promise.all([fCt0, fCt1, fd]));

        console.log('++++ test prepare statements ++++');
        var fp = db.prepare("Select datetime('now');INSERT OR REPLACE INTO COMPANY(ID,NAME,ADDRESS,Income)VALUES(?,?,?,?)");
        ...
        var fpi = db.execute(vParam, (data, proc, cols) => {
        ...
        fp = db.prepare("insert or replace into employee(EMPLOYEEID,CompanyId,name,JoinDate,image,DESCRIPTION,Salary)values(?,?,?,?,?,?,?)");
