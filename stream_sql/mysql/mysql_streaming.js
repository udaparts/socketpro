...
(async () => {
    try {
        ...
        var pCt0 = db.execute('Create database if not exists mysqldb character ' +
            'set utf8 collate utf8_general_ci;USE mysqldb');
        var pCt1 = db.execute('CREATE TABLE IF NOT EXISTS company(ID bigint PRIMARY KEY NOT NULL,name CHAR(64)' +
            'NOT NULL,ADDRESS varCHAR(256)not null,Income Decimal(21,2)not null)');
        var pCt2 = db.execute('CREATE TABLE IF NOT EXISTS employee(EMPLOYEEID bigint AUTO_INCREMENT PRIMARY KEY NOT ' +
        ...
        var pCt3 = db.execute('DROP PROCEDURE IF EXISTS sp_TestProc;CREATE PROCEDURE sp_TestProc(in p_company_id int,' +
        ...
        var ppi = db.execute(vParam);

        console.log('**** test prepare statements with long texts and blobs ****');
        var pp0 = db.prepare('insert into employee(CompanyId,name,JoinDate,image,DESCRIPTION,Salary)values(?,?,?,?,?,?)');
        ...
        var pp2 = db.prepare('call mysqldb.sp_TestProc(?,?,?)');
        var psp = db.execute(vParam, (data, proc, cols) => {
            console.log({ data: data, proc: proc, cols: cols });
        }, meta => {
            //console.log(meta);
        });
        ...
        buff.SaveObject('Donald Trump').SaveObject(new Date()).
            SaveObject(blob.PopBytes()).SaveObject(str, 'a').SaveObject(20254000);
        ...
        console.log(await Promise.all([pCt0, pCt1, pCt2, pCt3, pd, pp, ppi, pp0, ppi1, ps0, ps1, pp2, psp, pB]));
    } catch (ex) {
        console.log(ex);
    }
})();
