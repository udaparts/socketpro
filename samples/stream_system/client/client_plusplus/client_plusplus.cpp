...
int main(int argc, char* argv[]) {
    ...
    //lock one session, and all requests should be returned in sequence by wraping all requests between StartSequence and EndSequence
    handler = master.Lock();
    call_index = handler->StartSequence(nullptr);
    for (unsigned int n = 0; n < 1000; ++n) {
        call_index = handler->GetRentalDateTimes(n + 1, rdt);
    }
    call_index = handler->EndSequence(nullptr);
