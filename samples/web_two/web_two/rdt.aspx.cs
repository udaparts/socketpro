...
namespace web_two {
    public partial class CRdt : System.Web.UI.Page {
        ...
        private async Task ExecuteSql() {
            string sql = @"SELECT rental_id,rental_date,return_date,
                    last_update FROM rental where rental_id=" + txtRentalId.Text;
            ...
            var res = await handler.execute(sql, (h, v) => {
                txtResult.Text = string.Format(@"rental_id={0}, rental={1},
                    return={2}, lastupdate={3}", v[0], v[1], v[2], v[3]);
            });
            if (res.ec != 0)
